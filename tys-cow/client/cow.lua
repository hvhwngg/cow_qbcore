	...
	for i=1, #farmPlants, 1 do
		if GetDistanceBetweenCoords(coords, GetEntityCoords(farmPlants[i]), false) < 1.2 then
			...
			if nearbyObject and IsPedOnFoot(playerPed) then
                ...
                -- local circles = 1
                -- local success = exports['dc-ui']:StartLockPickCircle(circles, time, success)
                exports['dc-ui']:Circle(function(success)
                    if success then
...
RegisterNetEvent('lotus-milky:client:dobenxosua',function(slot)
    TriggerServerEvent('dc-doben:server:RemoveItemQuality',math.random(2,3),"xosua", slot)
end)

RegisterNetEvent('lotus-milky:client:dobenknife',function(slot)
    TriggerServerEvent('dc-doben:server:RemoveItemQuality',math.random(2,3),"knife", slot)
end)
...
Citizen.CreateThread(function()
    while true do
	        ...
	        if inPackingMilkStation then
                --exports['dc-core']:DrawText('[E] Đóng hộp', 'left')
...
exports['qb-target']:AddTargetModel(xosua, {
    options = {
        {
...
function killCowTime(time)
    ...
    Citizen.CreateThread(function()
        while canRob do
            --TriggerServerEvent('dc-hud:Server:Gain:Stress', 0.3) 
            TaskPlayAnim(PlayerPedId(), 'melee@hatchet@streamed_core', 'plyr_rear_takedown_b', 8.0, -8.0, -1, 2, 0, false, false, false)
            Citizen.Wait(2000)
            time = time - 2
            if time <= 0 then
           ...
           --TaskPlayAnim(PlayerPedId(), 'missfam1_yachtbattle', 'yacht_door_opening_mastout', 6.0, -6.0, -1, 46, 0, 0, 0, 0)
                QBCore.Functions.Progressbar('killing_cow', "Đang làm thịt", 10000, false, true, { -- Name | Label | Time | useWhileDead | canCancel
                    ...
                    disableCombat = true,
                        ...
                        --SpawnfarmPlants()
                        ClearPedTasksImmediately(PlayerPedId())
                     --   exports["dc-level"]:UpdateSkill("farming", 0.03)
                    end, function()
